...
veryOldInventors = inventors.filter(function (old, index) {
  if (old.year >= 1500 && old.year <= 1599) {
    return old;
  }
});
...
// Enter your solution code here:
inventorNames = inventors.map(function (firstLastName, index) {
  return /*inventorNames.push*/ [
    `first: ${firstLastName.first} `,
    ` last: ${firstLastName.last}`,
  ];
});
// Check your return value:
...
// Enter your solution code here:
sortedByBirthYear = inventors.sort((first, last) => first.year - last.year);
...
inventorNamedAda = inventors.find((name, index) => {
  if (name.first === "Ada") {
    return inventorNamedAda.push(name);
  }
});
// Check your return value:
...
// Enter your solution code here:
commentById = comments.find((comment , index)=>{
